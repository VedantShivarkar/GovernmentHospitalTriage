...
# In-memory store for user locations (phone -> (lat, lng, timestamp))
user_locations = {}
...
async def whatsapp_webhook(request: Request):
    ...
    # Case 1: Location message (no media, but has coordinates)
    if not media_url and latitude and longitude:
        user_locations[sender_phone] = (latitude, longitude, time.time())
        print(f"Stored location for {sender_phone}: {latitude}, {longitude}")
        twiml_response.message(
            "Location received. Now please send a VOICE NOTE describing your symptoms."
        )
        return Response(content=str(twiml_response), media_type="application/xml")

    # Case 2: Voice note message
    if media_url:
        # Check if we have a recent location for this user
        stored = user_locations.get(sender_phone)
        if stored and (time.time() - stored[2]) < 300:  # 5 minutes expiry
            latitude, longitude, _ = stored
            del user_locations[sender_phone]  # Use once
            print(f"Using stored location for {sender_phone}: {latitude}, {longitude}")
        else:
            print(f"No recent location for {sender_phone}")

        # Proceed with audio download and processing
        try:
            ...
            # Ambulance for priority 1 or 2 if location is available
            if triage_data["priority"] <= 2 and latitude and longitude:
                action = "dispatch_ambulance"
                ambulance_eta = "8 minutes"
                triage_data["clinical_summary"] = (
                    f"[AMBULANCE DISPATCHED to {latitude}, {longitude}] "
                ...
                reply_msg = "🚑 Ambulance sent. Please proceed to the hospital."
            else:
                reply_msg = (
    ...
    # Case 3: Neither location nor media (just text or something else)
    twiml_response.message(
        "Govt Hospital AI Triage: Please send a VOICE NOTE describing your symptoms."
    )
    return Response(content=str(twiml_response), media_type="application/xml")